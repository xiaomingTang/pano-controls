...
const loopedNumber = (val: number, min: number, max: number): number => val < min ? val + max - min : val > max ? val + min - max : val
...
class PanoControls extends THREE.EventDispatcher {
    ...
    update() {
        ...
        let rotateNeedUpdate = this.enableRotate && this.updateRotateDelta()
        let scaleNeedUpdate = this.enableScale && this.updateScaleDelta()
